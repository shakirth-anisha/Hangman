...
import pygame
import ttwidgets
from tkinter import Tk, END, CENTER, Text, PhotoImage, Label, DISABLED
import os
import random
from tkvideo import *
    ...
    def submit_txt():
        ...
        if chosen_word=="":
            ...
            input_box.bind('<Button-1>', click)
                ...
                input_box.insert("1.0", " ")
                input_box.tag_configure("center", justify=CENTER)
                input_box.insert("1.0", f"Enter \n letters only")
                input_box.tag_add("center", "1.0", "end")
                input_box.bind('<Button-1>', click)

        elif any(len(i)>13 for i in chosen_word.split(" ")) or chosen_word.count(" ")>5:
                input_box.delete("1.0", END)
                input_box.insert("1.0", " ")
                input_box.tag_configure("center", justify=CENTER)
...
def winner_screen():
    ...
    index_of_space = chosen_word.find(" ", 5)
...
def close_window(_event):
    hangman.destroy()
hangman.bind('<Escape>', close_window)
